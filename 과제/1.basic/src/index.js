...
const completeFolderTree = function (folderList) {
    for (let i = 0; i < folderList.length; i++) {
        folderList[i].addEventListener('click', (event) => {
            ...
            console.log(event.target.parentNode.querySelector('.folder'));
            console.log(event.target.parentNode.querySelectorAll('.lower-folder'));
            
            const toggle = event.target.parentNode;
            if(toggle.querySelectorAll('.folder') && toggle.classList.contains("isOpen")){
                toggle.querySelectorAll('.toggle').forEach((n) => {
                    n.classList.remove("isOpen");
                });
                toggle.querySelectorAll('.lower-folder').forEach((n) => {
                    n.style.display = 'none';
                });
            }
